            ...
            <Row gutter={[16, 16]} style={{ rowGap: "16px" }}>
              {description.map((item) => (
                <Col
                  ...
                  sm={{ span: 12 }}
                  key={item.key}
                  className="flex items-stretch justify-center" // Use flex and items-stretch
                >
                  <Descriptions
                    bordered
                    layout="vertical"
                    column={1}
                    // Remove max-h-[300px] and use h-full to make it take full height of its flex parent
                    className="inline-block xs:w-full h-full"
                    items={[
                      {
                        key: item.key,
                        label: item.label,
