...
export default function EditorLayoutClient({
  ...
  const debouncedBody = useMemo(() => ({ data: resumeData }), [resumeData]);
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        await fetch(`/api/resumes/${resumeId}`, {
          ...
          body: JSON.stringify(debouncedBody),
        });
      } catch {
      } finally {
  ...
  }, [debouncedBody, resumeId]);

  return (
    <div className="flex h-screen overflow-hidden bg-gray-50">
      {/* Left Sidebar - Content */}
      <div className="w-80 border-r bg-white flex-shrink-0 h-full overflow-y-auto shadow-sm">
        {Sidebar}
      </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Button
                ...
                icon={
                  showAIPanel ? <MenuFoldOutlined /> : <MenuUnfoldOutlined />
                }
                onClick={() => setShowAIPanel(!showAIPanel)}
                size="large"
                className={
                  showAIPanel ? "bg-violet-600 hover:bg-violet-700" : ""
                }
              >
                AI Insights
              </Button>
            ...
            <Button size="large" onClick={redo}>
              Redo
            </Button>
        ...
        {/* Canvas Content */}
        <div className="flex-1 overflow-y-auto">{Canvas}</div>
      </div>
      ...
      <AnimatePresence>
        {showAIPanel && (
          <motion.div
            ...
            className="border-l bg-white flex-shrink-0 h-full overflow-y-auto shadow-lg relative"
          >
            <Button
              type="text"
              icon={<CloseOutlined />}
